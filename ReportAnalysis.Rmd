...
Before running the below code, ensure that the relevant database has been created in your server. THe below code establishes a connection with the database.
...
This graph showcases the relationship between number of bookings and minimum night required to stay. As per the analysis, the demand for 1 night stay is the highest. However, the demand decreases as the minimum night required to stay increases. This trend ends when the minimum night of staying approaches to 30 days. From this analysis, we can conclude that the demand for 1-4 nights of staying is the highest, the Airbnb can recruit more hosts with the short-term available properties. On the other hand, Airbnb can also recruit more hosts with long-term (at least one-month) properties available.
